...
async def get_video_data(url, search, bettersearch, loop):
    if not has_voice:
        raise RuntimeError("nextcordUtils[voice] install needed in order to use voice")
...
def check_queue(ctx, opts, music, after, on_play, loop):
    ...
    if not song.is_looping:
        ...
        if len(music.queue[ctx.guild.id]) > 0:
            source = nextcord.PCMVolumeTransformer(nextcord.FFmpegPCMAudio(music.queue[ctx.guild.id][0].source, **opts))
...
class MusicPlayer(object):
    ...
    async def queue(self, url, search=False, bettersearch=False):
        ...
        return song
    async def play(self):
        source = nextcord.PCMVolumeTransformer(nextcord.FFmpegPCMAudio(self.music.queue[self.ctx.guild.id][0].source, **self.ffmpeg_opts))
